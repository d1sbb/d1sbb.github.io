...
layout: post
author: d1sbb
title: "DBeaver Ultimate edition 24.0.0 LicenseCrack破解(纯代码版)"
date: 2024-04-10
music-id: 
permalink: /archives/2024-04-10/1
description: "DBeaver-UE-24-Crack"
categories: "网络安全"
tags: [DBeaver, 数据库]
---
...
我用的环境是java17
## 安装
安装包`dbeaver-ue-24.0.0-x86_64-setup.exe`  
![Include Java](/assets/pic/DBeaverLicenseCrack/dbeaver1安装.png)
安装过程中可以取消`Include Java`内置的java版本
## 配置
打开安装根目录下的`dbeaver.ini`

1. 找到`plugins/org.eclipse.equinox.launcher.win32.win32.x86_64_1.2.800.v20231003-1442`在下面添加如下配置
```
-vm
你的java目录\jdk17\bin
```
2. 找到`-Ddbeaver.distribution.type=exe`在下面添加如下配置`-Dlm.debug.mode=true`保持配置文件

## 破解
到安装根目录的`\plugins`路径，找`org.jkiss.utils_2.2.0.202404011634.jar`和`com.dbeaver.lm.api_3.0.2.202404011634.jar`这两个包

创建个文件夹，在里面新建`DBeaverLicenseCrack.java`文件，并把上面2个jar包放到此文件夹下。   
在`DBeaverLicenseCrack.java`文件里添加如下代码：
```java
...
public class DBeaverLicenseCrack {
     ...
     */
    private static String replaceJarFile = "com.dbeaver.app.ultimate_24.0.0.202404011634.jar";

    private static File privateKeyFile = new File(new File(System.getProperty("user.home"), ".dbeaverCrack"), "private-key.txt");
    private static File publicKeyFile = new File(new File(System.getProperty("user.home"), ".dbeaverCrack"), "public-key.txt");
    private static File licenseFile = new File(new File(System.getProperty("user.home"), ".dbeaverCrack"), "license.txt");
    /**
     ...
     * </p>
     */
    private static File jarPublicKey = new File(new File(System.getProperty("user.home"), ".dbeaverCrack"), "dbeaver-ue-public.key");

    private static final String DBEAVER_ECLIPSE_HOME = "改成dbeaver的安装包路径";
...
```
![dbeaver2](/assets/pic/DBeaverLicenseCrack/dbeaver2找包.png)   
一定要改`DBeaverLicenseCrack.java`文件里内容`private static final String DBEAVER_ECLIPSE_HOME = 改成自己的安装包路径`

之后在自己的用户目录新建文件夹`.dbeaverCrack`
![dbeaver3](/assets/pic/DBeaverLicenseCrack/dbeaver3运行.png)  
一切配置好后，java17运行`DBeaverLicenseCrack.java`得到密钥，打开软件激活成功。
![dbeaver4](/assets/pic/DBeaverLicenseCrack/dbeaver4成功.png)  
仅供研究学习使用，请勿用于非法用途，请支持正版！

具体原理分析请看参考
> 参考文章1 [DBeaver Ultimate Edition License验证分析](https://www.52pojie.cn/thread-1668629-1-1.html)